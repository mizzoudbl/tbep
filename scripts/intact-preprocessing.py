...
def map_uniprot_to_gene(ref_file, interaction_file, output_file):
    hgnc_df = pd.read_csv(ref_file)[['UniProt ID(supplied by UniProt)', 'Approved symbol']]
    ...
    print(f'Checking for unordered duplicates (gene1,gene2 vs gene2,gene1)...')
...
map_uniprot_to_gene('data/hgnc_master_gene_list_with_uniprot.csv', 'data/human-unmapped.csv', 'data/intact_score.csv')